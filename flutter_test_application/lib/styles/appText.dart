import 'package:flutter/painting.dart';

class Apptext {
  static const header1 = TextStyle(fontSize: 24, fontWeight: FontWeight.w900);
  static const header2 = TextStyle(fontSize: 16, fontWeight: FontWeight.w600);
  static const subtitle1 = TextStyle(fontSize: 16, fontWeight: FontWeight.w600);
  static const subtitle2 = TextStyle(fontSize: 16, fontWeight: FontWeight.w500);
  static const subtitle3 = TextStyle(fontSize: 16, fontWeight: FontWeight.w400);
  static const body1 = TextStyle(fontSize: 14, fontWeight: FontWeight.w400);
  static const body2 = TextStyle(fontSize: 14, fontWeight: FontWeight.w500);
  static const body3 = TextStyle(fontSize: 16, fontWeight: FontWeight.w400);
}
